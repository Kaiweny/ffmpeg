...
enum Smpte2022FilterType {
    SMPTE2022_PES,
    SMPTE2022_SECTION,
    SMPTE2022_PCR,
};

typedef struct Smpte2022Filter Smpte2022Filter;

typedef int PESCallback (Smpte2022Filter *f, const uint8_t *buf, int len,
                         int is_start, int64_t pos);

typedef struct Smpte2022PESFilter {
    PESCallback *pes_cb;
    void *opaque;
} Smpte2022PESFilter;

typedef void SectionCallback (Smpte2022Filter *f, const uint8_t *buf, int len);

typedef void SetServiceCallback (void *opaque, int ret);

typedef struct Smpte2022SectionFilter {
    int section_index;
    int section_h_size;
    int last_ver;
    unsigned crc;
    ...
    void *opaque;
} Smpte2022SectionFilter;

struct Smpte2022Filter {
    int pid;
    int es_id;
    int last_cc; /* last cc code (-1 if first packet) */
    int64_t last_pcr;
    enum Smpte2022FilterType type;
    union {
        Smpte2022PESFilter pes_filter;
        Smpte2022SectionFilter section_filter;
    } u;
};
...
#define SIZE_STAT_THRESHOLD 10
    ...
    Smpte2022Filter *pids[NB_PID_MAX];
    int current_pid;
};

#define SMPTE2022_OPTIONS \
    { "resync_size",   "set size limit for looking up a new synchronization", offsetof(Smpte2022Context, resync_size), AV_OPT_TYPE_INT,  { .i64 =  MAX_RESYNC_SIZE}, 0, INT_MAX,  AV_OPT_FLAG_DECODING_PARAM }

static const AVOption options[] = {
    SMPTE2022_OPTIONS,
    {"fix_teletext_pts", "try to fix pts values of dvb teletext streams", offsetof(Smpte2022Context, fix_teletext_pts), AV_OPT_TYPE_BOOL,
     {.i64 = 1}, 0, 1, AV_OPT_FLAG_DECODING_PARAM },
    {"ts_packetsize", "output option carrying the raw packet size", offsetof(Smpte2022Context, raw_packet_size), AV_OPT_TYPE_INT,
     {.i64 = 0}, 0, 0, AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    {"scan_all_pmts",   "scan and combine all PMTs", offsetof(Smpte2022Context, scan_all_pmts), AV_OPT_TYPE_BOOL,
     { .i64 =  -1}, -1, 1,  AV_OPT_FLAG_DECODING_PARAM },
    {"skip_changes", "skip changing / adding streams / programs", offsetof(Smpte2022Context, skip_changes), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, 0 },
    {"skip_clear", "skip clearing programs", offsetof(Smpte2022Context, skip_clear), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, 0 },
    { NULL },
};

static const AVClass smpte2022_class = {
    .class_name = "smpte2022 demuxer",
...
static const AVOption raw_options[] = {
    ...
    { "compute_pcr",   "compute exact PCR for each transport stream packet",
          offsetof(Smpte2022Context, mpeg2ts_compute_pcr), AV_OPT_TYPE_BOOL,
          { .i64 = 0 }, 0, 1,  AV_OPT_FLAG_DECODING_PARAM },
    { "ts_packetsize", "output option carrying the raw packet size",
      offsetof(Smpte2022Context, raw_packet_size), AV_OPT_TYPE_INT,
      { .i64 = 0 }, 0, 0,
      AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    { NULL },
};

static const AVClass smpte2022raw_class = {
    .class_name = "smpte2022raw demuxer",
    .item_name  = av_default_item_name,
    .option     = raw_options,
    .version    = LIBAVUTIL_VERSION_INT,
};
...
extern AVInputFormat ff_smpte2022_demuxer;

static struct Program * get_program(Smpte2022Context *ts, unsigned int programid)
{
    int i;
...
static void clear_avprogram(Smpte2022Context *ts, unsigned int programid)
{
    AVProgram *prg = NULL;
    int i;
...
static void clear_program(Smpte2022Context *ts, unsigned int programid)
{
    int i;
...
static void clear_programs(Smpte2022Context *ts)
{
    av_freep(&ts->prg);
    ts->nb_prg = 0;
}

static void add_pat_entry(Smpte2022Context *ts, unsigned int programid)
{
    struct Program *p;
...
static void add_pid_to_pmt(Smpte2022Context *ts, unsigned int programid,
                           unsigned int pid)
{
...
static void set_pmt_found(Smpte2022Context *ts, unsigned int programid)
{
    struct Program *p = get_program(ts, programid);
    if (!p)
        return;
 ...
 *         0 otherwise
 */
static int discard_pid(Smpte2022Context *ts, unsigned int pid)
{
    int i, j, k;
 ...
 */
static void write_section_data(Smpte2022Context *ts, Smpte2022Filter *tss1,
                               const uint8_t *buf, int buf_size, int is_start)
{
    Smpte2022SectionFilter *tss = &tss1->u.section_filter;
    int len;
...
static Smpte2022Filter *smpte2022_open_filter(Smpte2022Context *ts, unsigned int pid,
                                        enum Smpte2022FilterType type)
{
    Smpte2022Filter *filter;

    av_log(ts->stream, AV_LOG_TRACE, "Filter: pid=0x%x type=%d\n", pid, type);

    if (pid >= NB_PID_MAX || ts->pids[pid])
        return NULL;
    filter = av_mallocz(sizeof(Smpte2022Filter));
    if (!filter)
        return NULL;
    ts->pids[pid] = filter;
...
static Smpte2022Filter *smpte2022_open_section_filter(Smpte2022Context *ts,
                                                unsigned int pid,
                                                SectionCallback *section_cb,
                                                void *opaque,
                                                int check_crc)
{
    Smpte2022Filter *filter;
    Smpte2022SectionFilter *sec;

    if (!(filter = smpte2022_open_filter(ts, pid, SMPTE2022_SECTION)))
        return NULL;
...
static Smpte2022Filter *smpte2022_open_pes_filter(Smpte2022Context *ts, unsigned int pid,
                                            PESCallback *pes_cb,
                                            void *opaque)
{
    Smpte2022Filter *filter;
    Smpte2022PESFilter *pes;

    if (!(filter = smpte2022_open_filter(ts, pid, SMPTE2022_PES)))
        return NULL;
...
static Smpte2022Filter *smpte2022_open_pcr_filter(Smpte2022Context *ts, unsigned int pid)
{
    return smpte2022_open_filter(ts, pid, SMPTE2022_PCR);
}

static void smpte2022_close_filter(Smpte2022Context *ts, Smpte2022Filter *filter)
{
    int pid;

    pid = filter->pid;
    if (filter->type == SMPTE2022_SECTION)
        av_freep(&filter->u.section_filter.section_buf);
    else if (filter->type == SMPTE2022_PES) {
...
static int skip_identical(const SectionHeader *h, Smpte2022SectionFilter *tssf)
{
...
static void smpte2022_find_stream_type(AVStream *st,
                                    uint32_t stream_type,
...
static int smpte2022_set_stream_info(AVStream *st, PESContext *pes,
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    smpte2022_find_stream_type(st, pes->stream_type, ISO_types);
    if (pes->stream_type == 4)
        st->request_probe = 50;
    if ((prog_reg_desc == AV_RL32("HDMV") ||
         prog_reg_desc == AV_RL32("HDPR")) &&
        st->codecpar->codec_id == AV_CODEC_ID_NONE) {
        smpte2022_find_stream_type(st, pes->stream_type, HDMV_types);
    ...
    if (st->codecpar->codec_id == AV_CODEC_ID_NONE)
        smpte2022_find_stream_type(st, pes->stream_type, MISC_types);
    if (st->codecpar->codec_id == AV_CODEC_ID_NONE) {
...
static int new_pes_packet(PESContext *pes, AVPacket *pkt)
{
    ...
    sd = av_packet_new_side_data(pkt, AV_PKT_DATA_SMPTE2022_STREAM_ID, 1);
    if (!sd)
        return AVERROR(ENOMEM);
    *sd = pes->stream_id;
...
static int smpte2022_push_data(Smpte2022Filter *filter,
                            const uint8_t *buf, int buf_size, int is_start,
                            int64_t pos)
{
    PESContext *pes   = filter->u.pes_filter.opaque;
    Smpte2022Context *ts = pes->ts;
    const uint8_t *p;
    int ret, len, code;
    ...
    if (is_start) {
        if (pes->state == SMPTE2022_PAYLOAD && pes->data_index > 0) {
        ...
        pes->state         = SMPTE2022_HEADER;
        pes->ts_packet_pos = pos;
    }
    p = buf;
    while (buf_size > 0) {
        switch (pes->state) {
        case SMPTE2022_HEADER:
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    if (!pes->st) {
                        ...
                        if (!pes->st)
                            return AVERROR(ENOMEM);
                        pes->st->id = pes->pid;
                        smpte2022_set_stream_info(pes->st, pes, 0, 0);
                    }
...
skip:
        ...
        case SMPTE2022_PESHEADER:
            ...
            if (pes->data_index == PES_HEADER_SIZE) {
                ...
                pes->state           = SMPTE2022_PESHEADER_FILL;
            }
            break;
        case SMPTE2022_PESHEADER_FILL:
                ...
                if (   pes->ts->fix_teletext_pts
                    ...
                    while ((p = av_find_program_from_stream(pes->stream, p, pes->st->index))) {
                        if (p->pcr_pid != -1 && p->discard != AVDISCARD_ALL) {
                            Smpte2022Filter *f = pes->ts->pids[p->pcr_pid];
                            if (f) {
                                AVStream *st = NULL;
                                if (f->type == SMPTE2022_PES) {
                                    PESContext *pcrpes = f->u.pes_filter.opaque;
                                    if (pcrpes)
                                        st = pcrpes->st;
                                } else if (f->type == SMPTE2022_PCR) {
                                    int i;
...
static PESContext *add_pes_stream(Smpte2022Context *ts, int pid, int pcr_pid)
{
    Smpte2022Filter *tss;
    PESContext *pes;
    ...
    pes->pts     = AV_NOPTS_VALUE;
    pes->dts     = AV_NOPTS_VALUE;
    tss          = smpte2022_open_pes_filter(ts, pid, smpte2022_push_data, pes);
    if (!tss) {
        av_free(pes);
        return 0;
    }
...
static void reanalyze(Smpte2022Context *ts) {
...
static int handle_packets(Smpte2022Context *ts, int64_t nb_packets)
{
    AVFormatContext *s = ts->stream;
    ...
    if (avio_tell(s->pb) != ts->last_pos) {
        ...
        for (i = 0; i < NB_PID_MAX; i++) {
            if (ts->pids[i]) {
                if (ts->pids[i]->type == SMPTE2022_PES) {
                    PESContext *pes = ts->pids[i]->u.pes_filter.opaque;
                    av_buffer_unref(&pes->buffer);
                    pes->data_index = 0;
                    pes->state = SMPTE2022_SKIP; /* skip until pes header */
                } else if (ts->pids[i]->type == SMPTE2022_SECTION) {
...
static int smpte2022_probe(AVProbeData *p)
{
...
static int smpte2022_read_header(AVFormatContext *s)
{
    Smpte2022Context *ts = s->priv_data;
    AVIOContext *pb   = s->pb;
    ...
    if (s->iformat == &ff_smpte2022_demuxer) {
        /* normal demux */
...
static int smpte2022_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (ret < 0) {
        ...
        /* flush pes data left */
        for (i = 0; i < NB_PID_MAX; i++)
            if (ts->pids[i] && ts->pids[i]->type == SMPTE2022_PES) {
                PESContext *pes = ts->pids[i]->u.pes_filter.opaque;
                if (pes->state == SMPTE2022_PAYLOAD && pes->data_index > 0) {
...
static void smpte2022_free(Smpte2022Context *ts)
{
    int i;
    ...
    for (i = 0; i < NB_PID_MAX; i++)
        if (ts->pids[i])
            smpte2022_close_filter(ts, ts->pids[i]);
}

static int smpte2022_read_close(AVFormatContext *s)
{
    Smpte2022Context *ts = s->priv_data;
    smpte2022_free(ts);
    return 0;
                              ...
                              int64_t *ppos, int64_t pos_limit)
{
    ...
    while(pos < pos_limit) {
        ...
        if (avio_read(s->pb, buf, TS_PACKET_SIZE) != TS_PACKET_SIZE)
            return AV_NOPTS_VALUE;
        if (buf[0] != 0x47) {
            if (smpte2022_resync(s, TS_PACKET_SIZE, buf) < 0)
                return AV_NOPTS_VALUE;
            pos = avio_tell(s->pb);
            continue;
        }
...
Smpte2022Context *avpriv_smpte2022_parse_open(AVFormatContext *s)
{
    Smpte2022Context *ts;

    ts = av_mallocz(sizeof(Smpte2022Context));
    if (!ts)
        return NULL;
 ...
 * packet is output */
int avpriv_smpte2022_parse_packet(Smpte2022Context *ts, AVPacket *pkt,
...
void avpriv_smpte2022_parse_close(Smpte2022Context *ts)
{
    smpte2022_free(ts);
    av_free(ts);
}

AVInputFormat ff_smpte2022_demuxer = {
    .name           = "smpte2022",
    .long_name      = NULL_IF_CONFIG_SMALL("MPEG-TS (MPEG-2 Transport Stream)"),
    .priv_data_size = sizeof(Smpte2022Context),
    .read_probe     = smpte2022_probe,
    .read_header    = smpte2022_read_header,
    .read_packet    = smpte2022_read_packet,
    .read_close     = smpte2022_read_close,
    .read_timestamp = smpte2022_get_dts,
    .flags          = AVFMT_SHOW_IDS | AVFMT_TS_DISCONT,
    .priv_class     = &smpte2022_class,
};